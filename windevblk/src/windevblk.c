    ...
    _Out_ LPDWORD pcReturned)
{
    ...
    cbNeeded = uiCount * sizeof(SMI_ENUMDRIVE_INFO);
    if (cbBuf < cbNeeded)
    {
        if (pcbNeeded)
        {
    ...
    else
    {
        if (pDriveInfoArray)
        {
            ...
            for (UINT i = 0; i < uiCount; i++)
            {
                ...
                if (pRawDevEntry)
                {   
                    PSMI_ENUMDRIVE_INFO pEnumDriveInfo = (PSMI_ENUMDRIVE_INFO)pDriveInfoArray;
                    ...
                    cbNeeded += sizeof(SMI_ENUMDRIVE_INFO);
                }
            }
            if (pcbNeeded) 
            { 
